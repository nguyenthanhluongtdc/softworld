...
class MStaffDao extends BaseDao {
	
	public function __construct() {
		parent::__construct("m_staff");
	}
	
	public function getUserNameById($staffId) {
		$sql = "SELECT staff_name FROM m_staff WHERE staff_id = :staff_id";
	...
	public function getWithPagging($currentPage, $pageSize, $sortCondition, &$totalRow, $params) {
		$sql = "SELECT m_staff.*,m_office_info.office_name FROM m_staff 
				LEFT JOIN m_office_info ON m_staff.staff_office_id = m_office_info.office_id 
				WHERE m_staff.deleted_flag <> 1";
		$andwhere = null;
		if (!StringUtil::isNullOrEmpty($params['staff_id'])) {
            $andwhere .= " AND m_staff.staff_id = :staff_id";
        }
        if (!StringUtil::isNullOrEmpty($params['staff_name'])) {
        	$params['staff_name'] = "%".$params['staff_name']."%";
            $andwhere .= " AND m_staff.staff_name LIKE :staff_name";
        }
        if (!StringUtil::isNullOrEmpty($params['staff_office_id'])) {
            $andwhere .= " AND staff_office_id = :staff_office_id";
        }
        if (!StringUtil::isNullOrEmpty($params['staff_department_id'])) {
            $andwhere .= " AND m_staff.staff_department_id = :staff_department_id";
        }
        if (!StringUtil::isNullOrEmpty($params['staff_phone_num'])) {
        	$params['staff_phone_num'] = "%".$params['staff_phone_num']."%";
            $andwhere .= " AND m_staff.staff_phone_num LIKE :staff_phone_num";
        }
        if (!StringUtil::isNullOrEmpty($params['staff_email'])) {
        	$params['staff_email'] = "%".$params['staff_email']."%";
            $andwhere .= " AND m_staff.staff_email LIKE :staff_email";
        }
        $sql .= $andwhere;
