...
//        document.getElementById(this.id).innerHTML = "none";
        }else{

        $('.check_class').prop("checked", this.checked);
        $('.check_class').parent().removeClass("checked"); //jquery ui
        }
...
$(document).ready(function(){
    ...
    $('ul.opensub').slideDown(500).addClass('drop');

    var _INTERFACE = $('#_INTERFACE').val();
    if($.cookie(_INTERFACE+'Img_header')){
        $('#header').css('background', 'url('+$.cookie(_INTERFACE+'Img_header')+')');
    }
    if($.cookie(_INTERFACE+'Img_sidebarbg')){
        $('#sidebarbg').css('background', 'url('+$.cookie(_INTERFACE+'Img_sidebarbg')+')');
    }
    if($.cookie(_INTERFACE+'Img_content')){
        $('#content').css('background', 'url('+$.cookie(_INTERFACE+'Img_content')+')');
    }
    ...
    mainNavLink.click(function(event) {
        ...
        if($this.hasClass('hasUl')) {
            ...
            if($this.hasClass('drop')) {
                ...
                //$('.mainnav').find('ul.sub').slideUp().siblings().removeClass('drop');
    ...
    $('.resBtn>a').click(function(event) {
        ...
        if($this.hasClass('drop')) {
            ...
            if(_CARRIER == 'sphone'){
                ...
                //$('#header').animate({ 'margin-left' : '500px'}, 300, function() {});
                $('#content').animate({ 'margin-left' : '211px'}, 300, function() {});
                //$('#content').animate({ 'margin-left' : '500px'}, 300, function() {});
            }
    ...
    $( '.collapseBtn' ).bind( 'click', function(){
        ...
        if ($this.hasClass('leftbar')) {
            ...
            if($(this).hasClass('hide')) {
                ...
                $('.collapseBtn.leftbar').css('top', '4'+'px').css('left', '170'+'px').removeClass('shadow');
                $this.children('a').attr('title','管理メニューを隠す');
                ...
                //expand content
                $this.addClass('hide');
                $this.children('a').attr('title','管理メニューを表示する');
                if($('#content').length) {
                    $('#content').css('margin-left', '0');
                }
                if($('#content-two').length) {
