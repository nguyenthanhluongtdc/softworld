...
class MEventDao extends BaseDao {
	...
	public function getEventById($id) {
		$sql = "SELECT * FROM e_event WHERE id = :id and deleted_flag <> 1";
		return $this->selectOne($sql, array("id" => $id));
	}

	function resetDeletedtag($type_id){
		return parent::update(
			array("deleted_flag" => 0)
			, array(
	...
	public function getWithPagging($currentPage, $pageSize, $sortCondition, &$totalRow, $params) {
		...
		if (!StringUtil::isNullOrEmpty($params['name_customer'])) {
        	$params['name_customer'] = "%".$params['name_customer']."%";
            $andwhere .= " AND e_event.name_customer LIKE :name_customer";
        }
		if (!StringUtil::isNullOrEmpty($params['phone_customer'])) {
        	$params['phone_customer'] = "%".$params['phone_customer']."%";
            $andwhere .= " AND e_event.phone_customer LIKE :phone_customer";
        }
		if (!StringUtil::isNullOrEmpty($params['email_customer'])) {
        	$params['email_customer'] = "%".$params['email_customer']."%";
            $andwhere .= " AND e_event.email_customer LIKE :email_customer";
        }
